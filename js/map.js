...
const excludedRadars = ["HWPA2", "TLKA2", "RODN", 'ROCO2'];
...
async function radars() {
    ...
    try {
        ...
        // Also check if the station data is stale (over 1 hour old).
        ...
        filteredRadarData = {
            ...
            features: data.features
                ...
                .map(feature => {
                    const props = feature.properties || {};
                    const lastReceivedTime = props.levelTwoLastReceivedTime;
                    let isStale = false;

                    // Check if the station is stale (over 1 hour old)
                    if (lastReceivedTime) {
                        const receivedTime = new Date(lastReceivedTime).getTime();
                        if (!isNaN(receivedTime) && receivedTime < oneHourAgo) {
                            isStale = true;
                        }
                    }
